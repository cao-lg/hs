...
last_id = 100
if match:
    all_ids = re.findall(r'id:\s*(\d+)', content)
...
for s in schools_to_add:
    ...
    nature = "民办" if s["type"] == "民办" else "公办"
...
if array_end_match:
    insert_pos = array_end_match.start()
    content = content[:insert_pos] + ",\n" + new_schools_js + content[insert_pos:]

with open(input_path, 'w', encoding='utf-8') as f:
    f.write(content)

print(f"Added {len(schools_to_add)} schools")
print(f"New IDs from {last_id + 1} to {current_id - 1}")
